...
## Decommissioned / Archived Functionality (Completed under SDNA-35)

1. Deterministic application-side Style-DNA trait inference/fallback behavior is retired in favor of strict-schema LLM-only trait inference.
...
2. DISC-003 residual limitation: submitted test-envelope evidence is validated server-side; image-upload digest persistence (`contentSha256`) and provenance receipt baseline fields (`provenanceSource`, `provenanceCapturedAtUtc`, `provenanceOperatorAssertion`) are now captured as immutable evidence context with strict-mode operator guidance/checklist documented, while external MidJourney render authenticity remains process-dependent.
3. Admin UX polish backlog is closed (Epic F post-epic items 1-5 completed): spacing/rhythm alignment, actionable warning copy normalization, run-detail readability pass, keyboard focus order/focus-return fixes, and expanded Playwright run-ops edge coverage are implemented and verified.
