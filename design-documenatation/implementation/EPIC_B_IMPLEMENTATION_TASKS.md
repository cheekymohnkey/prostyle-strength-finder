...
| B3 metadata extraction scaffold | `pass` | `scripts/ingestion/midjourney-metadata.js`, `scripts/ingestion/png-metadata.js`, `apps/frontend/src/index.js` (historical pre-Next migration); verified by frontend upload integration check (`POST /api/recommendation-extractions/upload` -> `201`) and smoke `pngFixture.path` output. Current UI runtime is Next App Router in `apps/frontend/app/*`. | Parser still depends on available PNG text/XMP metadata; keep troubleshooting guidance current. |
...
| B7/B8 API + frontend flow | `pass` | `apps/api/src/index.js`, `apps/frontend/src/index.js` (historical pre-Next migration); smoke command `set -a; source .env.local.example; set +a; npm run db:reset; npm run recommendation:smoke` returned `{ "ok": true }`; additional integration check for `POST /api/recommendation-extractions/upload` returned `201` with extraction payload. Current UI runtime is Next App Router in `apps/frontend/app/*`. | No blocking gaps for MVP-1 flow closure. |
